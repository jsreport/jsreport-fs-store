...
Persistence.prototype.persistNewState = function (newDocs, cb) {
  ...
  async.eachSeries(newDocs, function (doc, cb) {
    ...
    if (doc.$$deleted) {
      return fs.remove(self.entryDirectory(doc), function (err) {
        if (err) {
          return cb(err);
        }
...
Persistence.prototype.parseEntryDirectory = function (directory, cb) {
  ...
  fs.readFile(path.join(directory, 'config.json'), 'utf8', function (err, content) {
    if (err) {
      ...
      return cb(new Error('Unable to read config.json, you are likely trying to use fs jsreport store provider on the data set created with nedb provider. Please switch connection string name to nedb. \n' +
        err));
    }
