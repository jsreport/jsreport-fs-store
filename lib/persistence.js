...
async function parseAssets (fs, directory, files, model, folder) {
...
async function parseFiles (fs, parentDirectory, documentsModel, files) {
  ...
  if (!configFile) {
    ...
    return documentsModel.entitySets.assets ? [...(await parseAssets(fs, parentDirectory, files, documentsModel, folder)), folder] : [folder]
  }
  ...
  if (document.$entitySet === 'folders' && documentsModel.entitySets.assets) {
    return [...(await parseAssets(fs, parentDirectory, files, documentsModel, document)), document]
  }
