...
async function load (fs, directory, model, documents, parentDirectoryEntity) {
  ...
  const loadedDocument = await parseFiles(fs, directory, model, contentStats.filter((e) => !e.stat.isDirectory()).map(e => e.name))

  if (loadedDocument) {
    if (parentDirectoryEntity) {
      loadedDocument.folder = { shortid: parentDirectoryEntity.shortid }
    }
    if (loadedDocument.$entitySet === 'folders') {
      parentDirectoryEntity = loadedDocument
    }
