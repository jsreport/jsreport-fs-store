...
const { copy, deepGet, deepSet, deepDelete, serialize, parse, retry, uid, transactionDirectory } = require('./customUtils')
...
async function load (fs, directory, model, documents, parentDirectoryEntity) {
  ...
  const tranFiles = contentStats.filter((e) => !e.stat.isDirectory() && e.name.startsWith('fs.') && e.name.endsWith('.tran')).map((e) => e.name)

  for (const tranFile of tranFiles) {
    await fs.remove(fs.path.join(directory, tranFile))
  }
...
async function persist (fs, resolveFileExtension, model, doc, originalDoc, documents, transaction) {
  ...
  if (!model.entitySets[doc.$entitySet].splitIntoDirectories) {
    ...
    if (transaction) {
      docFinalPath = fs.path.join(transactionDirectory, docFinalPath)
    }
  ...
  if (transaction) {
    docInconsistentPath = fs.path.join(transactionDirectory, docInconsistentPath)
    docConsistentPath = fs.path.join(transactionDirectory, docConsistentPath)
    docFinalPath = fs.path.join(transactionDirectory, docFinalPath)
  }
  ...
  if (originalDoc && docClone.$entitySet === 'folders') {
    ...
    if (transaction) {
      originalDocPath = fs.path.join(transactionDirectory, originalDocPath)
    }
...
module.exports = ({ fs, documentsModel, corruptAlertThreshold, resolveFileExtension }) => ({
  update: (doc, originalDoc, documents, transaction) => lock(fs, () => persist(fs, resolveFileExtension, documentsModel, doc, originalDoc, documents, transaction)),
  insert: (doc, documents, transaction) => lock(fs, () => persist(fs, resolveFileExtension, documentsModel, doc, null, documents, transaction)),
  remove: (doc, documents, transaction) => lock(fs, () => remove(fs, documentsModel, doc, documents, transaction)),
  ...
  get transactionDirectory () {
    return transactionDirectory
  },
  getDocPath (doc, documents, transaction) {
    const entityType = documentsModel.entitySets[doc.$entitySet].entityType
    let docPath = fs.path.join(getDirectoryPath(fs, documentsModel, doc, documents), doc[entityType.publicKey])

    if (transaction) {
      docPath = fs.path.join(transactionDirectory, docPath)
    }
...
module.exports.transactionDirectory = transactionDirectory
