...
const { copy, deepGet, deepSet, deepDelete, serialize, parse, retry, uid, transactionInconsistentDirectory, transactionConsistentDirectory } = require('./customUtils')
...
async function persist (fs, resolveFileExtension, model, doc, originalDoc, documents, transaction) {
  if (!model.entitySets[doc.$entitySet].splitIntoDirectories) {
    let docFinalPath = doc.$entitySet

    if (transaction) {
      docFinalPath = fs.path.join(transactionInconsistentDirectory, docFinalPath)
    }
  ...
  let docInconsistentPath = fs.path.join(getDirectoryPath(fs, model, doc, documents), `~~${originalDocPrefix}${doc[entityType.publicKey]}`)
  let docConsistentPath = fs.path.join(getDirectoryPath(fs, model, doc, documents), `~${originalDocPrefix}${doc[entityType.publicKey]}`)
  let docFinalPath = fs.path.join(getDirectoryPath(fs, model, doc, documents), doc[entityType.publicKey])

  if (transaction) {
    docInconsistentPath = fs.path.join(transactionInconsistentDirectory, docInconsistentPath)
    docConsistentPath = fs.path.join(transactionInconsistentDirectory, docConsistentPath)
    docFinalPath = fs.path.join(transactionInconsistentDirectory, docFinalPath)
  }
  ...
  if (originalDoc && docClone.$entitySet === 'folders') {
    let originalDocPath = fs.path.join(getDirectoryPath(fs, model, originalDoc, documents), originalDoc[entityType.publicKey])

    if (transaction) {
      originalDocPath = fs.path.join(transactionInconsistentDirectory, originalDocPath)
    }
...
async function remove (fs, model, doc, documents, transaction) {
  if (!model.entitySets[doc.$entitySet].splitIntoDirectories) {
  ...
  const entityType = model.entitySets[doc.$entitySet].entityType
  let originalDocPath = fs.path.join(getDirectoryPath(fs, model, doc, documents), doc[entityType.publicKey])
...
module.exports = ({ fs, documentsModel, corruptAlertThreshold, resolveFileExtension }) => ({
  update: (doc, originalDoc, documents, transaction) => persist(fs, resolveFileExtension, documentsModel, doc, originalDoc, documents, transaction),
  insert: (doc, documents, transaction) => persist(fs, resolveFileExtension, documentsModel, doc, null, documents, transaction),
  remove: (doc, documents, transaction) => remove(fs, documentsModel, doc, documents, transaction),
  ...
  compact: (documents) => persistFlatCacheToFiles(fs, documentsModel, documents),
  get transactionInconsistentDirectory () {
    return transactionInconsistentDirectory
  },
  get transactionConsistentDirectory () {
    return transactionConsistentDirectory
  },
  copy: (source, target) => copy(fs, source, target, true),
  removeDirectory: (dir) => fs.remove(dir),
  renameDirectory: (source, target) => fs.rename(source, target),
  lock: (fn) => lock(fs, fn),
  exists: (dir) => fs.exists(dir)
})
