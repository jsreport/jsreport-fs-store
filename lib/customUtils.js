...
async function copy (fs, psource, ptarget, replace = false) {
  await fs.mkdir(ptarget)
  let dirEntries = await fs.readdir(psource)

  const filesFilter = (f) => {
    return (
      !f.startsWith(transactionInconsistentDirectory) &&
      !f.startsWith(transactionConsistentDirectory) &&
      f !== 'storage' &&
      f !== 'fs.lock'
    )
  }
...
module.exports.transactionInconsistentDirectory = transactionInconsistentDirectory
module.exports.transactionConsistentDirectory = transactionConsistentDirectory
