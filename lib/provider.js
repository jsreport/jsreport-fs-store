...
module.exports = ({ dataDirectory, logger, sync = {}, persistence = {}, syncModifications, compactionInterval = 5000 }) => ({
  name: 'fs',
  queue: Queue(),
  persistenceHandlers: {
  ...
  async update (entitySet, q, u, opts = {}) {
    ...
    const res = await this.queue(async () => {
      ...
      for (const doc of toUpdate) {
        await this.persistence.update(extend(true, {}, omit(doc, '$$etag'), u.$set || {}), doc)
