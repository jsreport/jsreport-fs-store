...
module.exports = ({
  ...
  find (entitySet, query, fields, options) {
    ...
    cursor.toArray = () => this.queue(async () => Promise.resolve(cursor.all().map((v) => extend(true, {}, omit(v, '$$etag', '$entitySet')))))
    return cursor
  },
  ...
  async update (entitySet, q, u, opts = {}) {
    ...
    const res = await this.queue(async () => {
      ...
      this._validateReadOnlyAssets(toUpdate, u)
  ...
  _validateReadOnlyAssets (toUpdate, update) {
    if (!this.AssetType) {
      return
    }

    for (const a of toUpdate.filter(a => a.$entitySet === 'assets')) {
      if (!a.isMetaReadOnly) {
        continue
      }

      const updatedAsset = extend(true, {}, a, update.$set)
      for (const prop in this.AssetType) {
        if (prop === 'content' || prop === 'modificationDate' || prop === '$$etag') {
          continue
        }

        if (!isEqual(updatedAsset[prop], a[prop])) {
          throw createError('Updating meta properties of readonly asset is forbiden.', {
            statusCode: 400,
            weak: true
          })
        }
