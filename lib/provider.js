...
module.exports = ({
  ...
  return {
    ...
    async load (model) {
      ...
      this.transaction = Transaction({ queue: Queue(persistenceQueueWaitingTimeout), persistence: this.persistence })
    ...
    insert (entitySet, doc, opts = {}) {
      return this.transaction.operation(opts, async (documents, persistence) => {
        doc._id = doc._id || uid(16)
        doc.$entitySet = entitySet

        await persistence.insert(doc, documents, opts.transaction)
    ...
    async update (entitySet, q, u, opts = {}) {
      ...
      const res = await this.transaction.operation(opts, async (documents, persistence) => {
        ...
        for (const doc of toUpdate) {
          await persistence.update(extend(true, {}, omit(doc, '$$etag'), u.$set || {}), doc, documents, opts.transaction)
    ...
    async remove (entitySet, q, opts = {}) {
      return this.transaction.operation(opts, async (documents, persistence) => {
        ...
        for (const doc of toRemove) {
          await persistence.remove(doc, documents, opts.transaction)
        }
    ...
    async _startCompactionInterval () {
      ...
      const compact = () => {
        ...
        return this.transaction.operation((documents) => Promise.resolve(this.persistence.compact(documents))).finally(() => (compactIsQueued = false))
      }
