...
module.exports = ({
  ...
  return {
    ...
    async load (model) {
      ...
      this.transaction = Transaction({ queue: Queue(persistenceQueueWaitingTimeout), persistence: this.persistence, fs: this.fs })
    ...
    async _startCompactionInterval () {
      ...
      const compact = () => {
        ...
        return Promise.resolve(this.transaction.operation((documents) => this.persistence.compact(documents))).finally(() => (compactIsQueued = false))
      }
