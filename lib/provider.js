...
module.exports = ({ dataDirectory, logger, sync = {}, persistence = {}, compactionInterval = 5000 }) => ({
  name: 'fs',
  queue: Queue(),
  persistenceHandlers: {
  ...
  async load (model) {
    ...
    const PersistenceProvider = this.persistenceHandlers[persistence.name || 'fs']
    if (!PersistenceProvider) {
      throw new Error(`File system store persistence provider ${persistence.name} was not registered`)
    }
    logger.info(`fs store is persisting using ${persistence.name || 'fs'}`)
    this.fs = PersistenceProvider(Object.assign({dataDirectory: dataDirectory}, persistence))

    if (!sync.name && persistence.name && persistence.name !== 'fs') {
      // no syncing by default for non fs based persistence
      logger.info(`fs store is using no synchronization`)
      this.sync = { subscribe: () => ({}), init: () => ({}) }
    } else {
      const Sync = this.syncHandlers[sync.name || 'fs']
      if (!Sync) {
        throw new Error(`File system store synchronization with ${sync.name} was not registered`)
      }

      logger.info(`fs store is synchronizing using ${sync.name || 'fs'}`)
      this.sync = Sync(Object.assign({dataDirectory: dataDirectory, fs: this.fs}, sync))
  ...
  find (entitySet, query, fields, options) {
    ...
    cursor.toArray = () => this.queue(async () => Promise.resolve(cursor.all().map((v) => omit(v, '$$etag', '$entitySet'))))
    return cursor
  },
  ...
  _startCompactionInterval () {
    this.autoCompactionInterval = setInterval(() => this.queue(() => this.persistence.compact()), compactionInterval).unref()
  }
})
