  ...
  init () {
    ...
    return new Promise(resolve => {
      ...
      this.watcher.on('all', (eventName, filePath, stat) => {
        ...
        function isInsideInterval (interval, t) {
          return t >= interval.start && (interval.end == null || t <= interval.end)
        }

        for (const interval of fs.lockIntervals) {
          if (isInsideInterval(interval, stat.mtime)) {
            return
          }
        }
