...
module.exports = ({ dataDirectory, lock }) => ({
  ...
  async transactionLock (withStaleCheck) {
    await mkdirpAsync(dataDirectory)
    // we don't use stale option by default because in transactions we want operation to always fail after retry is done
    return lockFile.lockAsync(path.join(dataDirectory, 'fs.tran'), Object.assign({}, withStaleCheck ? { stale: 18000 } : {}, transactionLockOptions))
  },
  releaseTransactionLock () {
    return lockFile.unlockAsync(path.join(dataDirectory, 'fs.tran'))
  }
})
