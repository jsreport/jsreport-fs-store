...
module.exports = ({ dataDirectory, lock }) => ({
  ...
  async lock () {
    ...
    // we keep lock intervals to be able to later in the chokidar determine if the write was inside
    // one of the locked intervals or not => it is our write or not

    // we need to use the time from the file because filesytem uses a different cached time then new Date
    // https://unix.stackexchange.com/questions/541386/file-modification-time-before-the-write-started
    const statLock = fs.statSync(path.join(dataDirectory, 'fs.lock'))
    this.currentLockInterval = {
      start: statLock.ctime
    }
